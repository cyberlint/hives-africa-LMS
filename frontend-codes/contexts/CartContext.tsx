...
interface CartContextValue {
  ...
  addItem: (item: Omit<CartCourseItem, "quantity"> & { quantity?: number }) => void;
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateQuantity = useCallback((courseId: string, quantity: number) => {
    setItems(prev => prev.map(p => p.id === courseId ? { ...p, quantity: Math.max(1, quantity) } : p));
  }, []);

  const clearCart = useCallback(() => setItems([]), []);

  const subtotal = useMemo(() => items.reduce((acc, i) => acc + (i.isFree ? 0 : i.unitPrice * i.quantity), 0), [items]);
